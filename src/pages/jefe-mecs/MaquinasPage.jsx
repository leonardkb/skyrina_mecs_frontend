...
export default function MaquinasPage() {
  ...
  const fetchMachines = async () => {
    try {
      setLoading(true)
      const response = await fetch(`http://localhost:8000/jefe-mecanicos/maquinas`, {
  ...
  const addMachine = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/jefe-mecanicos/maquinas?nombre=${encodeURIComponent(newMachine.trim())}`, {
        method: 'POST',
        headers: getAuthHeaders(),
      })
  ...
  const toggleMachineStatus = async (id) => {
    try {
      const response = await fetch(`http://localhost:8000/jefe-mecanicos/maquinas/${id}/toggle`, {
        method: 'PUT',
        headers: getAuthHeaders(),
      })
  ...
  const deleteMachine = async (id, nombre) => {
    if (window.confirm(`¿Eliminar la máquina "${nombre}"?`)) {
      try {
        const response = await fetch(`http://localhost:8000/jefe-mecanicos/maquinas/${id}`, {
          method: 'DELETE',
          headers: getAuthHeaders(),
        })
